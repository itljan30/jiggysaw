...
impl Puzzle {
    pub fn get_random(length: usize, height: usize, total_shapes: usize) -> Self {
        const UP: usize = 0;
        const RIGHT: usize = 1;
        const DOWN: usize = 2;
        const LEFT: usize = 3;
    ...
    pub fn get_solutions(&self, max_solutions: usize) -> Vec<Puzzle> {
    ...
    fn get_valid_next_pieces(
        ...
        for &piece in potential_pieces {
            ...
            for _ in 0..4 {
                ...
                if next_piece_type == PieceType::Edge {
                    if curr.len() < self.length || curr.len() >= self.length * (self.height - 1) {
                        if piece_copy.up != Edge::Edge && piece_copy.down != Edge::Edge {
                            continue;
                        }
                    }
                    else {
                        if piece_copy.left != Edge::Edge && piece_copy.right != Edge::Edge {
                            continue;
                        }
                    }
                }
                ...
                if !valid_pieces.contains(&piece_copy) {
